...
if ship:status = "ORBITING" {
		...
		if LandMode:contains("TARG") {
				if hastarget and TARGET:BODY = SHIP:BODY { // Make sure have a target in the same planet at least! Note it doesn't check if target is landed/splashed, will just use it's position, for all it cares.
						set LandLat to utilReduceTo360(TARGET:GEOPOSITION:LAT).
						set LandLng to utilReduceTo360(TARGET:GEOPOSITION:LNG).
				}
				else {
						set LandLat to utilReduceTo360(ship:geoposition:lat).
						set LandLng to utilReduceTo360(ship:geoposition:lng).
				}
		}
		else if LandMode:contains("COOR") {
				set LandLat to utilReduceTo360(LandLat).
				set LandLng to utilReduceTo360(LandLng).
		}
		...
		// JMT: extra debug
		uiBanner("Debug", "Site: " + LandingSite:lat + " " + LandingSite:lng).
		...
		Set Deorbit_Long to utilReduceTo360(LandLng - 180).
		Set PlaneChangeLong to utilReduceTo360(LandLng - 270).
		...
		// JMT: negative inclinations give negative values for dV
		if abs(TotIncDV) > 0.1 { // Only burn if it matters.
