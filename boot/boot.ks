...
// boot script
LOCAL FLS TO "". // LIST("lib_ui", "lib_parts").
...
FUNCTION INSTALL {
	PARAMETER FLS IS "".
	PARAMETER SRC IS PATH("0:/maps").
	IF FLS = "" {
		SET FLS TO OPEN(SRC):LIST():KEYS.
	}
	LOCAL NotFound TO LIST().
	LOCAL CopyFailed TO LIST().
	LOCAL DiskFull TO FALSE.
...
Print "Universal MAPS bootloader".
WAIT 1.
